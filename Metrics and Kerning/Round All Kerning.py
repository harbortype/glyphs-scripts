...
__doc__="""
Rounds all kerning pairs of the current master (or all masters) according a multiple provided by you.
"""
...
class RoundAllKerning(object):
	...
	def __init__(self):
		...
		# UI elements:
		self.w.text_1 = vanilla.TextBox((x, y, 216, self.textHeight), "Round all kerning pairs to multiples of", sizeStyle=self.sizeStyle)
		# y += self.textHeight

		# Multiple
		formatter = NSNumberFormatter.new()
		self.w.multiple = vanilla.EditText((216, y-3, -p, 19), "10", sizeStyle=self.sizeStyle, formatter=formatter, callback=self.SavePreferences)
		...
		# Run Button:
		self.w.runButton = vanilla.Button((80, -20 - 15, -80, -15), "Round kerning", sizeStyle='regular', callback=self.RoundAllKerningMain)
	...
	def SavePreferences(self, sender):
		try:
			Glyphs.defaults["com.harbortype.RoundAllKerning.multiple"] = self.w.multiple.get()
			# Glyphs.defaults["com.harbortype.RoundAllKerning.whichMasters"] = self.w.whichMasters.get()
		except:
			return False
	...
	def LoadPreferences(self):
		try:
			Glyphs.registerDefault("com.harbortype.RoundAllKerning.multiple", "10")
			Glyphs.registerDefault("com.harbortype.RoundAllKerning.whichMasters", 0)

			self.w.multiple.set(Glyphs.defaults["com.harbortype.RoundAllKerning.multiple"])
			self.w.whichMasters.set(bool(Glyphs.defaults["com.harbortype.RoundAllKerning.whichMasters"]))
		except:
			return False
	...
	def ProcessMaster(self, master_id):
		...
		for left_glyph, right_glyphs in kerning_dict.items():
			...
			for right_glyph, kerning_value in right_glyphs.items():
				...
				rounded_kerning = self.RoundKerningValue(kerning_value, self.multiple)
				if kerning_value != rounded_kerning:
					font.setKerningForPair(master_id, left_key, right_key, rounded_kerning)
